...
function Get-NewsItems {
    ...
    # Cap list and return
    return $distinct | Select-Object -First $MaxItems
...
function Start-NewsBar {
    param(
        ...
        [scriptblock]$RefreshScript,    # how to fetch new items
        [int]$RefreshMinutes = 0        # how often to refresh (0 = off)
    )
    ...
    # Reusable: convert an items array into ticker text
    $script:BuildTickerText = {
        
        param(
            [object[]]$arr,
            [switch]$WithDescriptions
        )

        $arr  = @($arr) # Force array semantics even if $arr is a single object
        if(-not $arr -or $arr.Count -eq 0){ return $null }
        
        # Make the separator using a current timestamp
        $sep = '  | NEWS @ ' + (Get-Date -Format 'HH:mm') + ' |  '
        
        
        # Bulid out the ticker text from each item title and description
        $L = @()
        foreach ($it in $arr){
            if(-not $it){ continue }
            $t = ($it.Title -replace '\s+', ' ').Trim()
            ...
            if($WithDescriptions -and $it.Description){
                $d = (($it.Description) -replace '\s+', ' ').Trim()
                if($d){ 
                    $L += "$t : $d"
                }else{
    ...
    $itemsLocal = @($Items) # Normalize incoming items array so a single item doesn't collaps the list.
    $textLocal = $script:BuildTickerText.Invoke($itemsLocal, $IncludeDescriptions.IsPresent)

    Write-Host ("[debug] itemsLocal={0} chars={1}" -f $itemsLocal.Count, ($textLocal?.Length)) -ForegroundColor DarkCyan
